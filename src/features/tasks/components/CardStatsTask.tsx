...
import { StatusTask, SubmitTaskStatus } from "../types/types";
...
export default function CardStatsTask({ title, status, icon, number}: Props) {
  return (
    <div className={clsx("border rounded-lg p-4 space-y-2", {
        ...
        'bg-yellow-50 border-yellow-600': status === 'entregada',
        'bg-green-50 border-green-600':  status === 'calificada' 
    })}>
        <div className="flex items-center justify-between">
            ...
            <span className={clsx("rounded-xl px-1 capitalize border-2", {
                ...
                'bg-yellow-100 text-yellow-600 border-yellow-600': status === 'entregada',
                'bg-green-100 text-green-600 border-green-600':  status === 'calificada' 
            })}>
                {status}
            </span>
        </div>
