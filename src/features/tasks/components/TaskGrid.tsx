...
export default async function TaskGrid({ userId }: Props) {
    ...
    const pending = taskList.filter((task)=> task.taskStatus === 'pendiente');
    const inProgress = taskList.filter((task)=> task.taskStatus === 'en progreso');
    const finished = taskList.filter((task)=> task.taskStatus === 'terminada');
